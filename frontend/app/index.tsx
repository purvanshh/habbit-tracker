...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (selectedHabit) {
      const needsWarning =
        !completedToday &&
        (selectedHabit.freezeTokens ?? 0) === 0 &&
        !selectedHabit.vacationMode;
      setBannerMessage(
        needsWarning ? "You're about to lose your streak" : null,
      );
    } else {
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <View style={{ marginLeft: 12 }}>
                <Text
                  style={{ color: "#6b7280", fontSize: 10, letterSpacing: 1 }}
                >
                  TOTAL STREAKS
                </Text>
                <Text
                  style={{ color: "#6366F1", fontSize: 28, fontWeight: "bold" }}
                >
                  {totalStreaks}
                </Text>
              </View>
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <View style={{ marginLeft: 12 }}>
                ...
                <Text
                  style={{ color: "#A855F7", fontSize: 28, fontWeight: "bold" }}
                >
                  {habits.length}
                </Text>
              </View>
              ...
              renderItem={({ item, index }) => (
                <AnimatedTouchable
                  ...
                  style={{
                    ...
                    borderWidth: 1,
                    borderColor:
                      selectedHabitId === item.id ? "#6366F1" : "#222",
                    flexDirection: "row",
                    alignItems: "center",
                  }}
                >
                  <Ionicons
                    name={(item.icon || "barbell") as any}
                    size={16}
                    color={selectedHabitId === item.id ? "#6366F1" : "#9ca3af"}
                    style={{ marginRight: 8 }}
                  />
                  <Text
                    style={{
                      color:
                        selectedHabitId === item.id ? "#6366F1" : "#9ca3af",
                    }}
                  >
                    {item.name}
                  </Text>
              ...
              <>
                <HabitCard
                  ...
                  onEdit={() =>
                    router.push({
                      pathname: "/edit-habit",
                      params: { id: selectedHabit.id },
                    } as any)
                  }
                  onDelete={() => {
                    removeHabit(selectedHabit.id);
                    setSelectedHabitId(
                      habits.find((h) => h.id !== selectedHabit.id)?.id || null,
                    );
                  }}
                />
                ...
                >
                  ...
                  >
                    ...
                    >
                      {selectedHabit.freezeTokens ?? 0} Freeze Tokens
                    </Text>
                    <TouchableOpacity
                      onPress={() => useFreezeToken(selectedHabit.id)}
                      disabled={(selectedHabit.freezeTokens ?? 0) === 0}
                      style={{
                        backgroundColor:
                          (selectedHabit.freezeTokens ?? 0) === 0
                            ? "#1f2937"
                  ...
                  >
                    ...
                    <TouchableOpacity
                      onPress={() => activateVacation(selectedHabit.id)}
                      disabled={selectedHabit.vacationMode}
                      style={{
                        backgroundColor: selectedHabit.vacationMode
                          ? "#1f2937"
                          : "#10b981",
                        paddingHorizontal: 12,
                        paddingVertical: 8,
                ...
                <Animated.View
                  entering={FadeInUp.delay(320).duration(220)}
                  style={{ marginTop: 16 }}
                >
                  <TouchableOpacity
      ...
      {selectedHabit && !completedToday && (
        ...
        >
          <SwipeSlider
            ...
            canSkip={
              (selectedHabit.skipsUsedThisWeek || 0) <
              (selectedHabit.maxSkipsPerWeek || 2)
            }
          />
        </Animated.View>
      )}
        ...
        >
          <View
            style={{
              ...
              borderWidth: 1,
              borderColor:
                completionStatus === "skipped" ? "#F59E0B" : "#6366F1",
            }}
          >
            <Ionicons
              name={
              ...
              size={20}
              color={completionStatus === "skipped" ? "#F59E0B" : "#6366F1"}
            />
            <Text
              style={{
                color: completionStatus === "skipped" ? "#F59E0B" : "#6366F1",
